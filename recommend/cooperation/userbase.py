...
class UserBase:
    ...
    def recommend(self, reviews, targetReviewerId, asinList):
        ...
        for reviewerId in reviews:
            ...
            reviews[targetReviewerId] = targetReviewerReviewVector
            reviews[reviewerId] = reviewVector
        ...
        for i in range(len(asinList)):
            ...
            if asinList[i] not in reviews:
                ...
                recommendedItems[asinList[i]] =sum([reviews[tup[0]][i] * tup[1] for tup in topSimilarities]) * normalizingConstant
    ...
    def createReviewVector(self, perReviewerReviews, asinList):
        return np.array([perReviewerReviews[asin] if asin in perReviewerReviews else 0 for asin in asinList])